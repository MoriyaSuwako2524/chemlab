...
class single_spin_job(base_job):
    ...
    def generate_outputs(self, new_file_name="",prefix="./"):
        filename = self.xyz_name if new_file_name == "" else new_file_name
        ref = self._prepare_ref()
...
class qchem_out_excite_multi(qchem_out_multi):
    ...
    def export_gradients(self, prefix="", grad_unit=("hartree", "bohr"),state_idx=1,np_save=False):
        ...
        return gradients
    def export_transition_density(self, prefix="S", unit="e", state_idx=1,np_save=False):
        transition_density = self.export_attr(
            extractor=lambda st, task: np.array(st.esp_transition_density, dtype=float)
            if st.esp_transition_density is not None else None,
            shape_func=lambda natoms, nframes: (nframes, natoms),
            state_idx=state_idx,
        )

        esp_transition_density = transition_density
        if np_save:
    ...
    def export_gs_esp_charge(self, prefix="gs", unit="e", state_idx=0,np_save=False):
        esp_charge = self.export_attr(
            ...
            if st.esp_charge is not None else None,
            shape_func=lambda natoms, nframes: (nframes, natoms),
            state_idx=state_idx,
        )
        if np_save:
